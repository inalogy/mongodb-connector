...
public class MongoDbConnector implements
    ...
    public void test() {
        try {
            ...
            if (templateUser != null) {
                ...
                LOG.info("Test successful, but no templateUser found.");
            }
        } catch (Exception e) {
        ...
        for (AttributeDelta delta : deltas) {
            ...
            if (valuesToAdd != null && !valuesToAdd.isEmpty()) {
                if (this.configuration.isUseSetForUpdates()) {
                    ...
                    LOG.ok("Using pushEach for valuesToAdd modification");
    ...
    public void sync(ObjectClass objectClass, SyncToken syncToken, SyncResultsHandler syncResultsHandler, OperationOptions operationOptions) {
        LOG.ok("Starting sync operation for object class {0} with sync token: {1}", objectClass, syncToken);

        if (!objectClass.is(ObjectClass.ACCOUNT_NAME)) {
            LOG.error("Unsupported object class: {0}", objectClass.getObjectClassValue());
            throw new ConnectorException("Unsupported object class: " + objectClass.getObjectClassValue());
        }

        String syncAttribute = this.configuration.getLiveSyncAttributeName();
        if (syncAttribute == null || syncAttribute.isEmpty()) {
            throw new ConnectorException("LiveSync attribute name is not configured.");
        }

        if (syncToken == null) {
            LOG.ok("Empty sync token, fetching latest sync token.");
        ...
        Object tokenValue = syncToken.getValue();
        Instant instant = Instant.parse((String) tokenValue);
        Date lastSyncValue = Date.from(instant);

        LOG.ok("Sync starting from token value: {0}", lastSyncValue);

        FindIterable<Document> users = this.connection.getSyncUsers(syncAttribute, lastSyncValue, null, null);

        Date lastProcessedValue = lastSyncValue;

        for (Document document : users) {
            SyncDeltaBuilder builder = new SyncDeltaBuilder();

            lastProcessedValue = (Date) document.get(syncAttribute);
            builder.setToken(new SyncToken(lastProcessedValue.toInstant().toString()));
            ConnectorObject connectorObject = SchemaHandler.convertDocumentToConnectorObject(document, schema, objectClass, this.configuration);
            builder.setDeltaType(SyncDeltaType.CREATE_OR_UPDATE);
            builder.setObject(connectorObject);

            if (!syncResultsHandler.handle(builder.build())) {
                LOG.error("Sync handler interrupted, stopping sync process.");
                return;
            }
        }

        LOG.ok("LiveSync completed successfully. Last processed value: {0}", lastProcessedValue);
    ...
    public SyncToken getLatestSyncToken(ObjectClass objectClass) {
        LOG.ok("Fetching latest sync token for object class: {0}", objectClass);
        ...
        Object latestToken = this.connection.getLatestSyncToken(syncAttribute);
        Date isoDate = (Date) latestToken;

        LOG.ok("Latest sync token value retrieved: {0}", latestToken);

        return new SyncToken(isoDate.toInstant().toString());
    }
